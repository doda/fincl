...
__all__ = ['DATA_DIR', 'F_PAYLOAD_DIR', 'DAILY_DATA_DIR', 'get_symbols', 'load_contract', 'load_contracts',
           'load_all_cont_contracts', 'get_data', 'process_bars', 'load_and_sample_bars', 'determine_bar_size',
           'load_hdf', 'save_hdf', 'bars_path', 'events_b_path', 'feats_path', 'imp_path', 'payload_path', 'load_bars',
           'save_bars', 'load_events_b', 'save_events_b', 'load_feats', 'save_feats', 'load_imp', 'save_imp',
...
from mlfinlab.data_structures import get_dollar_bars, get_tick_bars, get_time_bars
...
def get_symbols(symbol_groups):
    ...
    picked = s_csv[s_csv["Sector"].isin(symbol_groups)]
...
def imp_path(symbol, c):
    feat_names = '-'.join(sorted(set(x.split('_')[0] for x in c['features'])))
    return DATA_DIR / c['bar_type'] / f"{symbol}_imp_{c['binarize']}_{c['binarize_params']}_{c['alpha']}_{c['alpha_params']}_{feat_names}_{c['feat_imp_method']}.h5"
...
def load_feats(symbol, config):
    path = feats_path(symbol, config)
    return load_hdf(path)


def save_feats(symbol, config, feats):
    path = feats_path(symbol, config)
    return save_hdf(feats, path)
