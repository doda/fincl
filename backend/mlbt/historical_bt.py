...
__all__ = ['simulate_pnl', 'vola_reducer', 'estimate_trading_costs', 'SLIPPAGE_ESTIMATE', 'COMMISSION_ESTIMATE', 'RAW',
           'RAWWW', 'RAA', 'RAA']
...
RAW = RISK_ADJUSTMENT_WINDOW = 22 * 350 # monthly vola for 350 bars per day
RAWWW = RISK_ADJUSTMENT_WINDOW_LONG = RAW * 5
RAA = RISK_ADJUSTMENT_FACTOR = 0.5
RAA = RISK_ADJUSTMENT_FACTOR = 1

def simulate_pnl(config, close, signal, pos_size=50000, pos_cap_multi=500, init_capital=7e7, adjust_vola=False):
    pos_cap = pos_size * pos_cap_multi
    volatility = np.log(close).diff().ewm(com=32 * 25).std()
    prices = (np.log(close).diff() / volatility).cumsum()

    currency_pos_pre = (pos_size * signal / volatility).clip(-pos_cap, pos_cap)
    profits_gross_pre = (close.pct_change() * currency_pos_pre.shift(periods=1)).sum(axis=1)

    if adjust_vola:
        nav_gross_pre = (1 + profits_gross_pre / init_capital).cumprod()
        vola_reducer_gross = vola_reducer(nav_gross_pre)
        vola_reducer_gross.plot()
        currency_pos = (pos_size * signal / volatility).mul(1 + vola_reducer_gross, axis=0).clip(-pos_cap, pos_cap)
        profits_gross = (close.pct_change() * currency_pos.shift(periods=1)).sum(axis=1)
    else:
        profits_gross = profits_gross_pre
...
def vola_reducer(nav, periods=RAW, periods_l=RAWWW, mult_factor=RAA):
    ...
    std_nav_vol = (std_nav_vol).clip(-2,2) * -0.5 * mult_factor # clip to 2 stdev and invert to create portfolio adjustor
    import ipdb;ipdb.set_trace()

    return std_nav_vol.reindex(nav.index).fillna(0)
