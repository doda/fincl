...
__all__ = ['roll_measure', 'roll_impact', 'kyle', 'amihud', 'autocorr', 'rolling_stdev', 'ewm_stdev', 'ewm_mean',
           'int_ret', 'log_ret', 'ffd', 'volratio', 'binary_encoding', 'quantile_encoding', 'sigma_encoding', 'entropy',
           'close', 'lag', 'lag_change', 'lag_diff', 'ema', 'sadf', 'month', 'week', 'day', 'weekday', 'hour',
           'tick_bars', 'run_feature_engineering', 'get_bars', 'fill_out_symbol', 'engineer_feature', 'compute_feature',
           'define_feature_configs', 'SYMBOLS_CSV', 'SYMBOLS_DICT', 'ENCODERS', 'ENTROPY_FUNS', 'FEATURES']
...
def sadf(df, model="linear", min_length=20, lags=5, column="Close"):
    return get_sadf(df[column], model=model, add_const=True, min_length=min_length, lags=lags, num_threads=2)
...
#         sadf,
    ...
    for window in windows:
        ...
        # Volatilty
        features.append({"name": "ewm_stdev", "com": window, "symbol": log_ret_vix})
        ...
        ewm_stdev_log_ret = {"name": "ewm_stdev", "com": window, "symbol": log_ret}

        features.append(rolling_stdev_log_ret)
        features.append(ewm_stdev_log_ret)

        features.append({"name": "rolling_stdev", "window": window, "symbol": rolling_stdev_log_ret})
        features.append({"name": "ewm_stdev", "com": window, "symbol": ewm_stdev_log_ret})

        features.append({"name": "rolling_stdev", "window": window, "symbol": ffd_f})
        features.append({"name": "ewm_stdev", "com": window, "symbol": ffd_f})
...
#                 features.append({"name": "auto", "window": window, "lag": lag, "symbol": stdev_volratio})
    ...
    for window in windows_ent:
        ...
        konto_len = (window // 10) + 1
        features.append({"name": "entropy", "method": "konto", "encoding": "sigma", "window": window, "symbol": log_ret, "konto_len": konto_len})
